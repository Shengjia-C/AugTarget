...
class AGPCNet(nn.Module):
    def __init__(self, backbone='resnet18', scales=(10, 6), reduce_ratios=(8, 8), gca_type='patch', gca_att='origin',
        ...
        ##############################################################
#         self.fc_encode = nn.Linear(1000,128)
#         self.alpha = 1.0
    ...
    def forward(self, x):
        ...
        c1, c2, c3, c4 = self.backbone(x) #c4: 8,1000
        ...
        pred = self.head(out) #8, 128, 128, 128
        out = F.interpolate(pred, size=[hei, wid], mode='bilinear', align_corners=True) #8, 1, 256, 256
        
        ##############################################################
        if self.training:
            # feat = self.fc_encode(c4)
